
// Dark Oak Trapdoor
(item('quark:dark_oak_trapdoor') * 6).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak')],
    [item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak')]
)

// Birch Trapdoor
(item('quark:birch_trapdoor') * 6).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch')],
    [item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch')]
)

// Acacia Trapdoor
(item('quark:acacia_trapdoor') * 6).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia')],
    [item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia')]
)

// Jungle Trapdoor
(item('quark:jungle_trapdoor') * 6).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle')],
    [item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle')]
)

// Cyan Stained Planks
(item('quark:stained_planks', 9) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeCyan'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Lime Stained Planks
(item('quark:stained_planks', 5) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeLime'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Pink Stained Planks
(item('quark:stained_planks', 6) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyePink'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Gray Stained Planks
(item('quark:stained_planks', 7) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeGray'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Light Gray Stained Planks
(item('quark:stained_planks', 8) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeLightGray'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Orange Stained Planks
(item('quark:stained_planks', 1) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeOrange'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Magenta Stained Planks
(item('quark:stained_planks', 2) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeMagenta'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Light Blue Stained Planks
(item('quark:stained_planks', 3) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeLightBlue'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Yellow Stained Planks
(item('quark:stained_planks', 4) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeYellow'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Spruce Bookshelf
item('quark:custom_bookshelf').tweakRecipe(
    [item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce')],
    [item('minecraft:book'), item('minecraft:book'), item('minecraft:book')],
    [item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce')]
)

// Stone Speleothem
(item('quark:stone_speleothem') * 6).tweakRecipe(
    [ore('stones'), ore('stones'), ore('stones')]
)

// Birch Bookshelf
item('quark:custom_bookshelf', 1).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch')],
    [item('minecraft:book'), item('minecraft:book'), item('minecraft:book')],
    [item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch'), item('contenttweaker:lumber_vanilla_birch')]
)

// Dark Oak Bookshelf
item('quark:custom_bookshelf', 4).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak')],
    [item('minecraft:book'), item('minecraft:book'), item('minecraft:book')],
    [item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak'), item('contenttweaker:lumber_vanilla_dark_oak')]
)

// Acacia Bookshelf
item('quark:custom_bookshelf', 3).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia')],
    [item('minecraft:book'), item('minecraft:book'), item('minecraft:book')],
    [item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia'), item('contenttweaker:lumber_vanilla_acacia')]
)

// Jungle Bookshelf
item('quark:custom_bookshelf', 2).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle')],
    [item('minecraft:book'), item('minecraft:book'), item('minecraft:book')],
    [item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle'), item('contenttweaker:lumber_vanilla_jungle')]
)

// Spruce Trapdoor
(item('quark:spruce_trapdoor') * 6).tweakRecipe(
    [item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce')],
    [item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce'), item('contenttweaker:lumber_vanilla_spruce')]
)

// Red Stained Planks
(item('quark:stained_planks', 14) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeRed'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Black Stained Planks
(item('quark:stained_planks', 15) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeBlack'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// White Stained Planks
(item('quark:stained_planks') * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeWhite'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Purple Stained Planks
(item('quark:stained_planks', 10) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyePurple'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Blue Stained Planks
(item('quark:stained_planks', 11) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeBlue'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Brown Stained Planks
(item('quark:stained_planks', 12) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeBrown'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Green Stained Planks
(item('quark:stained_planks', 13) * 8).tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('lumber'), ore('dyeGreen'), ore('lumber')],
    [ore('lumber'), ore('lumber'), ore('lumber')]
)

// Horse Whistle
item('quark:horse_whistle').tweakRecipe(
    [ore('lumber'), ore('lumber'), ore('lumber')],
    [ore('nuggetIron'), ore('stickWood'), ore('lumber')]
)
