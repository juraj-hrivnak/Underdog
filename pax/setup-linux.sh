...
# Parse json
pax_version=$(grep -Po '"PAX":[\s]*"\K[^"]*' setup.json)
modpack_downloader_version=$(grep -Po '"ModpackDownloader":[\s]*"\K[^"]*' setup.json)
server_pack_creator_version=$(grep -Po '"ServerPackCreator":[\s]*"\K[^"]*' setup.json)
