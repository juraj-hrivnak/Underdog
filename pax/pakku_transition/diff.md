```diff
+ Better Line Break
+ File Director
+ JEI Area Fixer
+ JourneyMapStages Continuation
+ MiniEffects-Legacy Extended Life
+ NeverEnoughAnimation
+ Somnia Refreshed
+ 籁/Sound Physics Remixin
- Fantastic Lib
- Item Stages
- JourneyMapStages
- MultiBlock Stages
- No Advancements
- Shadowfacts' Forgelin
- Somnia Awoken
! appliedenergistics2-rv6-stable-7-extended_life-v0.55.25.jar -> ae2-uel-v0.56.6.jar
! Alfheim-1.0.2.jar                                           -> Alfheim-1.4.jar
! AquaAcrobatics-1.15.3.jar                                   -> AquaAcrobatics-1.15.4.jar
! Bundles-1.1.jar                                             -> Bundles-1.21.jar
! !configanytime-1.0.jar                                      -> !configanytime-3.0.jar
! Controlling-3.0.12.2.jar                                    -> Controlling-3.0.12.4.jar
! CraftTweaker2-1.12-4.1.20.695.jar                           -> CraftTweaker2-1.12-4.1.20.700.jar
! DramaticTrees-1.12.2-0.13.2.jar                             -> DramaticTrees-1.12.2-0.13.4.jar
! dropt-1.12.2-1.19.3.jar                                     -> dropt-1.12.2-1.19.4.jar
! EntityCulling-1.12.2-6.4.1.jar                              -> EntityCulling-1.12.2-6.4.3.jar
! Forgelin-Continuous-1.9.0.0.jar                             -> Forgelin-Continuous-2.0.10.0.jar
! future-mc-0.2.11.jar                                        -> Future-MC-0.2.20.jar
! Greenery-1.12.2-4.5.jar                                     -> Greenery-1.12.2-5.1.jar
! groovyscript-0.6.4.jar                                      -> groovyscript-1.1.3.jar
! HadEnoughItems_1.12.2-4.25.1.jar                            -> HadEnoughItems_1.12.2-4.25.5.jar
! bogosorter-1.4.4.jar                                        -> bogosorter-1.4.9.jar
! journeymap-1.12.2-5.5.4.jar                                 -> journeymap-1.12.2-5.7.1p2.jar
! LittleTiles_v1.5.82_mc1.12.2.jar                            -> LittleTiles_v1.5.87_mc1.12.2.jar
! logisticspipes-0.10.4.47.jar                                -> logisticspipes-0.10.4.49.jar
! censoredasm5.18.jar                                         -> censoredasm5.20.jar
! MCTImmersiveTechnology-1.12.2-1.9.101.jar                   -> MCTImmersiveTechnology-1.12.2-1.9.102.jar
! !mixinbooter-8.6.jar                                        -> !mixinbooter-9.3.jar
! DrZharks-MoCreatures-Extended-12.3.1.jar                    -> DrZharks-MoCreatures-Extended-12.4.5.jar
! modularui-2.2.3.jar                                         -> modularui-2.4.3.jar
! NotEnoughEnergistics-1.12.2-2.0.6.jar                       -> NotEnoughEnergistics-1.12.2-2.0.7.jar
! particleculling-1.12.2-v1.4.1.jar                           -> particleculling-1.12.2-v1.4.3.jar
! Patchouli-1.0-25.jar                                        -> Patchouli-1.0-26.jar
! pyrotech-1.12.2-1.6.9.jar                                   -> pyrotech-1.12.2-1.6.16.jar
! QuarkRotN-r1.6-188.jar                                      -> QuarkRotN-r1.6-189.jar
! ReachFix-1.12.2-1.0.9.jar                                   -> ReachFix-1.12.2-1.0.10.jar
! RecurrentComplex-1.4.8.5.jar                                -> RecurrentComplex-1.4.8.6.jar
! !Red-Core-MC-1.7-1.12-0.5-Dev-3.jar                         -> !Red-Core-MC-1.8-1.12-0.6-Dev-7.jar
! RenderLib-1.12.2-1.3.3.jar                                  -> RenderLib-1.12.2-1.3.5.jar
! serializationisbad-1.5.jar                                  -> serializationisbad-1.5.2.jar
! voicechat-forge-1.12.2-2.4.27.jar                           -> voicechat-forge-1.12.2-2.5.21.jar
! Sledgehammer-1.12.2-2.0.23.jar                              -> Sledgehammer-1.12.2-2.0.26.jar
! startuptimer-1.0.1.jar                                      -> startuptimer-1.1.0.jar
! StorageDrawers-1.12.2-5.5.0.jar                             -> StorageDrawers-1.12.2-5.5.1.jar
! tweakedlib-1.0.1.jar                                        -> tweakedlib-2.0.0.jar
! tweakedpetroleum-1.3.0.jar                                  -> tweakedpetroleum-2.0.1.jar
! UniversalTweaks-1.12.2-1.8.0.jar                            -> UniversalTweaks-1.12.2-1.12.0.jar
! vintagefix-0.3.3.jar                                        -> vintagefix-0.5.1.jar
! Fixeroo-2.0.jar                                             -> Fixeroo-2.3.jar
! zenutils-1.14.15.jar                                        -> zenutils-1.20.4.jar
```
